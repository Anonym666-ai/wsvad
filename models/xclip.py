...
class XCLIP(CLIP):
    def __init__(self,
        ...
        # self.video_fc = nn.Linear(embed_dim, 64)
        self.head_max = nn.Linear(embed_dim, embed_dim)
        # self.head_mar = nn.Linear(64, embed_dim)
        self.head_u = nn.Linear(embed_dim, embed_dim)
        # self.head_text = nn.Linear(embed_dim, embed_dim)
        # self.u_head_text = nn.Linear(embed_dim, embed_dim)
    ...
    def uda(self, video_feature, text_feature, train_flag):
        v_fea = self.head_max(self.ln_head(video_feature))
        v_fea_u = self.head_u(self.ln_head(video_feature))
        ...
        if train_flag:
            v_fea_nograd = self.head_max(self.ln_head(video_feature.detach()))
            v_fea_u_nograd = self.head_u(self.ln_head(video_feature.detach()))
            v_fea_nograd = v_fea_nograd / v_fea_nograd.norm(dim=-1, keepdim=True)
            v_fea_u_nograd = v_fea_u_nograd / v_fea_u_nograd.norm(dim=-1, keepdim=True)

            t_fea_nograd = t_fea.detach()
            return video_feature, v_fea, v_fea_u, t_fea, v_fea_nograd, v_fea_u_nograd, t_fea_nograd
        else:
    ...
    def forward(self, image, text):
        ...
        if self.training:
            video_features, v_features, v_features_u, t_features, \
                v_features_n, v_features_u_n, t_features_n = self.uda(video_features, text_features, self.training)
            ...
            logits_n = torch.einsum("bd,bkd->bk", v_features_n, logit_scale * t_features_n)
            logits_u_n = torch.einsum("bd,bkd->bk", v_features_u_n, logit_scale * t_features_n)
            ...
            video_features, v_features, t_features, v_features_u, t_features_u= self.uda(video_features, text_features, self.training)
            logits = torch.einsum("bd,bkd->bk", v_features, logit_scale * t_features)
            logits_u = torch.einsum("bd,bkd->bk", v_features_u, logit_scale * t_features_u)
