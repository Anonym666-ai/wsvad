...
def parse_option():
    ...
    parser.add_argument('--w-con1', default=0.0, type=float, help='weight of clustering all consistency loss')
...
def main(config):
    ...
    for epoch in range(start_epoch, config.TRAIN.EPOCHS):
        ...
        train_one_epoch(epoch, model, optimizer, lr_scheduler, train_loader, text_labels, config, data_dict)
...
def train_one_epoch(epoch, model, optimizer, lr_scheduler, train_loader, text_labels, config, data_dict):
    model.train()
    ...
    for idx, batch_data in enumerate(train_loader):
        ...
        bz = images.shape[0]
        n_aug = images.shape[1]
        n_clip = images.shape[2]
        # import pdb;pdb.set_trace()
        ...
        logits = rearrange(output['y'], '(b a k) c -> (b a) k c', b=bz, a=n_aug,)
        # logits_alt = rearrange(output['y_cluster_all'], '(b a k) c -> (b a) k c', b=bz, a=n_aug, )
        ...
        # import pdb;pdb.set_trace()
        scores_bz = rearrange(scores, '(b a) k c -> b a k c', b=bz, a=n_aug, )
        # scores_alt_bz = rearrange(scores_alt, '(b a) k c -> b a k c', b=bz, a=n_aug, )

        logits_bz = rearrange(logits, '(b a) k c -> b a k c', b=bz, a=n_aug, )
        # logits_alt_bz = rearrange(logits_alt, '(b a) k c -> b a k c', b=bz, a=n_aug, )

        max_prob, pseudo_labels = torch.max(scores_bz[:, 0], dim=-1)
        # max_prob_alt, pseudo_labels_alt = torch.max(scores_alt_bz[:, 0], dim=-1)

        consistency_loss = (
                    F.cross_entropy(logits_bz[:, 1].contiguous().view(-1, 2), pseudo_labels.view(-1), reduction='none') \
                    * max_prob.view(-1).ge(args.threshold).float().detach()).mean()

        # Cluster consistency loss
        # consistency_loss_alt = (
        #             F.cross_entropy(logits_alt_bz[:, 1].contiguous().view(-1, 2), pseudo_labels_alt.view(-1),
        #                             reduction='none') \
        ...
        total_loss = loss_mil + consistency_loss * w_con + smoothed_loss * w_smooth + sparsity_loss * w_sparse
        ...
        cst_loss_meter.update((consistency_loss * args.w_con).item(), len(label_id))
...
def validate(data_loader, text_labels, model, config, out_path):
    ...
    with torch.no_grad():
        ...
        for idx, batch_data in enumerate(data_loader):
            ...
            scores_cls = F.softmax(output['y'], dim=-1)
