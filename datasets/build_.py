            ...
            for line in fin:
                ...
                if self.multi_class:
                    ...
                    if len(line_split)==4:
                        ...
                        filename, end, label,_,_,_,_ = line_split
                        start = 0
                    label = int(label)
    ...
    def load_annotations(self):
        ...
        for x in open(self.ann_file):
            ...
            for i in range(num_segs):
                ...
                if end - start < 5:
                    continue
                seg = dict(frame_dir=video_info.path, label=video_info.label, start=int(start), total_frames=int(end)-int(start), tar=self.use_tar_format, vid=vid)
                segs.append(seg)
            vid += 1
            # import pdb;pdb.set_trace()
...
def build_dataloader(logger, config):
    ...
    val_pipeline = [
        dict(type='SampleFrames', clip_len=config.DATA.NUM_FRAMES, frame_interval=config.DATA.FRAME_INTERVAL, num_clips=config.DATA.NUM_CLIPS*4, test_mode=True),
        dict(type='RawFrameDecode'),
        dict(type='Resize', scale=(-1, scale_resize)),
        dict(type='CenterCrop', crop_size=config.DATA.INPUT_SIZE),
        dict(type='Normalize', **img_norm_cfg),
